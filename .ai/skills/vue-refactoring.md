...
- **Static Analysis**: Use ESLint/Prettier (from [vue-project-setup.md](./vue-project-setup.md)) and TypeScript compiler for errors/warnings.
- **Metrics**: Check line counts, dependencies, and coupling. Tools like SonarQube or VS Code extensions can help.
- **Manual Review**: Read code for smells (e.g., long methods, magic numbers per [vue-constants-standard.md](./vue-constants-standard.md)).
...
- **Step-by-Step Approach**:
  ...
  2. **Backup**: Ensure tests pass (see [vue-unit-testing.md](./vue-unit-testing.md)).
  3. **Plan Changes**: List steps (e.g., "Extract function X to composable Y").
  4. **Implement Incrementally**: Change small parts, test each.
  5. **Validate**: Run full suite, check performance (see [vue-performance.md](./vue-performance.md)).
- **Risk Mitigation**: Use feature flags or branches. Rollback if issues.
- **Example Plan for a Large Component**:
  - Step 1: Extract API logic to [vue-api-integration.md](./vue-api-integration.md) composable.
  - Step 2: Split UI into atoms/molecules per [vue-atomic-design.md](./vue-atomic-design.md).
  - Step 3: Add tests for new parts.
  - Step 4: Remove old code after validation.

## 5. Integration with Your Skills
- Combine with [vue-coding-standards.md](./vue-coding-standards.md) for style.
- Ensure tests cover refactored code (see below).
- Document changes in commits per [git-standards.md](./git-standards.md).
